...
thresholds1a = (58, 81, 7, 86, 32, 82)#ボール捕捉エリア
thresholds_g = (40, 100, -128, 127, -128, 127)#コート色
thresholds_b = (39, 59, -21, 8, -55, -22)#青ゴール
#thresholds_y = (64, 74, -25, -4, 30, 72)#黄色ゴール
thresholds_y = (81, 100, -26, -2, 39, 73)#黄色ゴール
...
sensor.set_auto_exposure(False, exposure_us=15000)#大会4500
"""
...
tateline=151#画面上での縦線の座標 白162　黒152
point_675 = [
...
def check_green(points):
    ...
    for x, y, d in points:
        ...
        if (thresholds_g[0] <= pixel[0] <= thresholds_g[1] and \
            ...
            thresholds_g[4] <= pixel[2] <= thresholds_g[5]) and \
        not(thresholds_b[0] <= pixel[0] <= thresholds_b[1] and \
            thresholds_b[2] <= pixel[1] <= thresholds_b[3] and \
            thresholds_b[4] <= pixel[2] <= thresholds_b[5]) and \
        not(thresholds_y[0] <= pixel[0] <= thresholds_y[1] and \
...
while(True):
    kick_area = [tateline-17,yokoline-65,34,14]#[x,y,w,h]
    areaA=areaB=areaC=areaD=areaLine=0#色の面積の最大値を配列にして保存
    rectB=rectC=rectD=rectLine=0#取得した中心のX座標、Y座標、ブロックの横幅、縦幅の値を配列に
    cxB=cxC=cxD=0#取得したx座標保存
    cyB=cyC=cyD=0#取得したY座標のみを保存
    valk=val=val2=valc=vald=0#画面中心を原点としたときのｘ座標
    vallk=vall=vall2=vallc=valld=0#画面中心を原点としたときのｙ座標
    radsk=rads2=rads3=rads4=0#ｒによって出た角度を絶対値に
    kick = 0;
    distance = 115
    distance_b = 0
    distance_y = 0
    ...
    if not img == None:
        ...
        if cxA!=0:#ボールの場所を画面に表示
            if DESIGN:
                img.draw_rectangle(rectA,(255,0,0))#長方形の生成
                img.draw_cross(cxA, cyA) #中央のバツの生成
                if areaA == 0:
                    img.draw_line(tateline,235,cxA,cyA,(255,128,0))
                else:
                    img.draw_line(tateline,235,cxA,cyA,(255,255,255))
        ...
        if DESIGN:
            ...
            img.draw_circle(BX,BY,BSR,(0, 255, 255))
            img.draw_circle(BX,BY,BLR,(0, 255, 255))
