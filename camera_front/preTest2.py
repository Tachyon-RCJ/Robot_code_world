...
#thresholds----------------------------------------------------------------------------
#o:橙 g:緑 b:青 y:黄
thresholds1 = (47, 85, 6, 50, 46, 85)#ボールの色
#thresholds1 = [(44, 74, -36, -8, -25, 11)]
thresholds1a = (82, 100, -16, 8, -4, 19)#ボール捕捉エリア
thresholds_g = (0, 100, -128, -19, -26, 127)#コート色
thresholds_b = (65, 74, -36, -6, -47, -23)#青ゴール
thresholds_y = (40, 65, -4, 12, 41, 77)#黄色ゴール
...
shootMove = False
last_kick_time = 0
yokoline=110#画面上での横線の座標170
tateline=168#画面上での縦線の座標
point_675 = [
...
while(True):
    ...
    if not img == None:
        ...
        if blobs:
            for b in blobs:
                if b.area() > areaA and not(tateline-65 < b.cx() < tateline+65 and yokoline-30 < b.cy() < yokoline+80) and math.sqrt((b.cx()-tateline)**2+(b.cy()-yokoline)**2) < 140:
                    areaA = b.area()
                    cxA=b.cx()
                    cyA=b.cy()
                    rectA=b.rect()
    ...
    #Design===============================================================================
        if areaA!=0:#ボールの場所を画面に表示
            ...
            if tateline-25 <= cxA <= tateline+25 and \
                yokoline-73 <= cyA <= yokoline-63:
                current_time = time.time()
                if current_time - last_kick_time >= 0.5:
                    kick = 1
