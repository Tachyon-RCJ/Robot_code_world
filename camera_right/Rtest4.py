...
#thresholds----------------------------------------------------------------------------
#o:橙 g:緑 b:青 y:黄
thresholds_o = (73, 86, -11, 21, 78, 90)
thresholds_g = (29, 73, -54, -17, -15, 48)
...
sensor.set_auto_whitebal(False,(65, 61, 120))
sensor.run(1)
...
yokoline=240#画面上での横線の座標
tateline=290#画面上での縦線の座標
DX=0
DY=0
POINT180 = [
(248+DX,141+DY,10), (256+DX,114+DY,20), (260+DX,99+DY,30), (263+DX,90+DY,40), (264+DX,85+DY,50),
(265+DX,80+DY,60), (266+DX,77+DY,70), (267+DX,74+DY,80), (268+DX,72+DY,90), (268+DX,71+DY,100)
]
POINT1575 = [
(212+DX,124+DY,10), (215+DX,94+DY,20), (216+DX,77+DY,30), (217+DX,67+DY,40), (217+DX,61+DY,50),
(218+DX,56+DY,60), (218+DX,52+DY,70), (218+DX,50+DY,80), (218+DX,48+DY,90), (218+DX,46+DY,100)
]
POINT135 = [
(173+DX,118+DY,10), (169+DX,86+DY,20), (167+DX,70+DY,30), (166+DX,59+DY,40), (165+DX,52+DY,50),
(164+DX,47+DY,60), (164+DX,43+DY,70), (164+DX,41+DY,80), (164+DX,39+DY,90), (163+DX,37+DY,100)
]
POINT1125 = [
(132+DX,122+DY,10), (122+DX,91+DY,20), (116+DX,73+DY,30), (113+DX,64+DY,40), (111+DX,57+DY,50),
(109+DX,53+DY,60), (108+DX,50+DY,70), (107+DX,47+DY,80), (107+DX,45+DY,90), (106+DX,43+DY,100)
]
POINT90 = [
(93+DX,139+DY,10), (77+DX,110+DY,20), (68+DX,94+DY,30), (63+DX,85+DY,40), (59+DX,79+DY,50),
(57+DX,75+DY,60), (56+DX,72+DY,70), (54+DX,69+DY,80), (53+DX,67+DY,90), (52+DX,66+DY,100)
]
...
def check_green(points):
    ...
    for x, y, d in points:
        for dx in range(-1, 2):
            for dy in range(-1, 2):
                pixel = image.rgb_to_lab(img1.get_pixel(x + dx, y + dy))
                pixel_sum[0] += pixel[0]
                pixel_sum[1] += pixel[1]
                pixel_sum[2] += pixel[2]
        pixel_sum[0] /= 9
        ...
        if thresholds_b[0] <= pixel[0] <= thresholds_b[1] and \
           ...
           thresholds_b[4] <= pixel[2] <= thresholds_b[5]:
            img1.draw_circle(x,y,2,(255, 255, 0))
            ex_b = 1;
            ...
            img1.draw_circle(x,y,2,(0, 0, 255))
            ex_y = 2;
            ...
            img1.draw_circle(x,y,2,(0, 255, 0))
            distance_max = max(distance_max,d)
        else:
            img1.draw_circle(x,y,2,(255, 0, 0))
...
def loop1():
    ...
    while True:
        #cameraLiDAR===================================================================
        if not img1 == None:
            court_max[0] = check_green(POINT180)#180度
            court_max[1] = check_green(POINT1575)#157.5度
            court_max[2] = check_green(POINT135)#135度
            court_max[3] = check_green(POINT1125)#112.5度
            court_max[4] = check_green(POINT90)#90度
            time.sleep_ms(1)
        #==============================================================================
#--------------------------------------------------------------------------------------
#start_loop1---------------------------------------------------------------------------
...
while True:
    ...
    if not img1 == None:
        ...
        if areaO == 0:
            ...
            elif 0 <= time.ticks_diff(time.ticks_ms(),time_stamp) - startTime <= 1500:
                pass
            else:
                rectO=cxO=cyO=radsO=pixel1=0
